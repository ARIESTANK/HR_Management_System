...
type Department = "Engineering" | "Marketing" | "Sales" | "HR" | "Finance" | "Design";
type Tab        = "personal" | "job" | "attendance" | "payroll";
...
/* ── Seed data ──────────────────────────────────────────────────── */
const SEED: Employee[] = [
  { id:"1", name:"Sarah Chen",    employeeId:"EMP-001", department:"Engineering", role:"Senior Engineer",    status:"Active",     email:"sarah.chen@acme.com",    phone:"+1 555-0101", hireDate:"Mar 12, 2021", salary:115000, attendanceRate:98 },
  { id:"2", name:"Mark Wilson",   employeeId:"EMP-002", department:"Marketing",   role:"Marketing Lead",    status:"Active",     email:"mark.wilson@acme.com",   phone:"+1 555-0102", hireDate:"Jul 5, 2020",  salary:88000,  attendanceRate:95 },
  { id:"3", name:"Priya Patel",   employeeId:"EMP-003", department:"Sales",       role:"Account Executive", status:"On Leave",   email:"priya.patel@acme.com",   phone:"+1 555-0103", hireDate:"Jan 20, 2022", salary:72000,  attendanceRate:91 },
  { id:"4", name:"Alex Rivera",   employeeId:"EMP-004", department:"Design",      role:"UX Designer",       status:"Active",     email:"alex.rivera@acme.com",   phone:"+1 555-0104", hireDate:"Nov 3, 2019",  salary:94000,  attendanceRate:97 },
  { id:"5", name:"Jordan Kim",    employeeId:"EMP-005", department:"Finance",     role:"Financial Analyst", status:"Active",     email:"jordan.kim@acme.com",    phone:"+1 555-0105", hireDate:"Sep 14, 2023", salary:81000,  attendanceRate:99 },
  { id:"6", name:"Taylor Brooks", employeeId:"EMP-006", department:"HR",          role:"HR Specialist",     status:"Terminated", email:"taylor.brooks@acme.com", phone:"+1 555-0106", hireDate:"Feb 28, 2018", salary:67000,  attendanceRate:88 },
];

const SEED_LEAVES: LeaveRequest[] = [
  { id:"l1", type:"Annual",  from:"2025-03-10", to:"2025-03-12", days:3,  reason:"Family vacation",         status:"Approved", appliedOn:"2025-02-28" },
  { id:"l2", type:"Sick",    from:"2025-01-15", to:"2025-01-16", days:2,  reason:"Fever and rest",          status:"Approved", appliedOn:"2025-01-15" },
  { id:"l3", type:"Casual",  from:"2025-04-22", to:"2025-04-22", days:1,  reason:"Personal errand",         status:"Pending",  appliedOn:"2025-04-20" },
  { id:"l4", type:"Unpaid",  from:"2024-12-26", to:"2024-12-27", days:2,  reason:"Extended holidays",       status:"Rejected", appliedOn:"2024-12-20" },
];
...
/* ── Constants ──────────────────────────────────────────────────── */
const DEPT_COLORS: Record<Department, { from:string; to:string; light:string; text:string }> = {
  Engineering:{ from:"#6366f1", to:"#8b5cf6", light:"#eef2ff", text:"#6366f1" },
  Marketing:  { from:"#ec4899", to:"#f43f5e", light:"#fdf2f8", text:"#ec4899" },
  Sales:      { from:"#059669", to:"#10b981", light:"#ecfdf5", text:"#059669" },
  HR:         { from:"#f59e0b", to:"#f97316", light:"#fffbeb", text:"#d97706" },
  Finance:    { from:"#0ea5e9", to:"#6366f1", light:"#f0f9ff", text:"#0284c7" },
  Design:     { from:"#8b5cf6", to:"#d946ef", light:"#faf5ff", text:"#7c3aed" },
};
...
const LEAVE_STATUS_STYLES: Record<LeaveStatus,{ bg:string; color:string }> = {
...
const DEPARTMENTS: Department[] = ["Engineering","Marketing","Sales","HR","Finance","Design"];
...
const TABS: { key:Tab; label:string; icon:string }[] = [
  ...
  { key:"leaveRequest" , label:"LeaveRequest" , icon:"📝"}
];
...
function StatusBadge({ status:Status }) {
  ...
  return (
    <span style={{ display:"inline-flex", alignItems:"center", gap:5, padding:"3px 10px", borderRadius:999,  color:"#246368", fontSize:11.5, fontWeight:600 }}>
...
function RequestLeaveModal({ emp, leaves, onClose, onSubmit }:{
  ...
  const balance = { Personal:20-usedAnnual, Sick:12, "Others":90 };

  const handleSubmit = async (e:React.FormEvent) => {
    e.preventDefault();
    setErr("");
    if (!reason.trim()) { setErr("Please provide a reason."); return; }
    if (new Date(to) < new Date(from)) { setErr("End date must be after start date."); return; }
    if (balance[type] < days) { setErr(`Insufficient ${type} leave balance (${balance[type]} days left).`); return; }
    const response = await fetch("http://localhost:8080/leaves/create",{
      method:"POST",
      headers:{ "Content-Type":"application/json" },
      body:JSON.stringify({ "employeeId": emp.employeeID, "reason":type, "leaveDate":from,"status":"Pending" })
    })
    // console.log(type);
    ...
    onSubmit({
      id: `l${Date.now()}`, type, from, to, days, reason,
      ...
      <div style={{ ...MODAL_BOX, maxWidth:500, maxHeight:"92vh", overflowY:"auto" }}>
        ...
        <div style={{ margin:"0 26px 20px", display:"flex", gap:8, flexWrap:"wrap" }}>
          {(Object.entries(balance) as [LeaveType,number][]).map(([t,b])=>(
            <div key={t} style={{ padding:"6px 12px", borderRadius:8, background: type===t?"#eef2ff":"#faf9ff", border:`1px solid ${type===t?"#6366f1":"#e8e7f3"}`, cursor:"pointer", transition:"all 0.15s" }} onClick={()=>setType(t)}>
              <p style={{ margin:0, fontSize:10, fontWeight:700, color:type===t?"#6366f1":"#94a3b8", textTransform:"uppercase", letterSpacing:"0.05em" }}>{t}</p>
              <p style={{ margin:0, fontSize:14, fontWeight:800, color:type===t?"#6366f1":"#1e1b4b" }}>{b}d</p>
            </div>
          ))}
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ padding:"0 26px 26px", display:"flex", flexDirection:"column", gap:14 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 14px", borderRadius:10, background:"linear-gradient(135deg,#eef2ff,#faf5ff)", border:"1px solid #c7d2fe" }}>
            ...
            <div>
              <p style={{ margin:0, fontSize:12, color:"#6366f1", fontWeight:700 }}>{days} working day{days!==1?"s":""}</p>
              <p style={{ margin:0, fontSize:11.5, color:"#8b5cf6" }}>{fmtDate(from)} → {fmtDate(from)}</p>
            </div>
          </div>
...
function CommitAttendanceModal({ emp, attendance, onClose, onSubmit }:{
  ...
  const alreadyLogged = attendance.some(a => a.date.includes(new Date(date+"T00:00:00").toDateString().slice(4)));
  ...
  const handleSubmit = async (e:React.FormEvent) => {
    e.preventDefault();
    setErr("");
    if (checkOut <= checkIn) { setErr("Check-out must be after check-in time."); return; }
            ...
            return (
              <div key={s} onClick={()=>setStatus(s)}
                style={{ padding:"10px 14px", borderRadius:10, cursor:"pointer",
                  background: status===s ? st.bg : "#faf9ff",
                  border:`1.5px solid ${status===s ? st.color : "#e8e7f3"}`,
        ...
        <form onSubmit={handleSubmit} style={{ padding:"0 26px 26px", display:"flex", flexDirection:"column", gap:14 }}>
          <div>
            ...
            <input type="date" value={date} max={todayStr()} onChange={e=>setDate(e.target.value)} style={inputSt}
              onFocus={e=>e.target.style.borderColor="#6366f1"} onBlur={e=>e.target.style.borderColor="#e8e7f3"} />
            <p style={{ margin:"5px 0 0", fontSize:12, color:"#94a3b8" }}>{fmtDateLabel(date)}</p>
          </div>
          ...
          {alreadyLogged && (
            <div style={{ padding:"10px 14px", borderRadius:10, background:"#fffbeb", border:"1px solid #fde68a", color:"#d97706", fontSize:13, fontWeight:500 }}>
              ⚠️ Attendance for this date is already logged. Submitting will add a new record.
            </div>
          )}
...
/* ══════════════════════════════════════════════════════════════════
   MODAL 3 — PAYROLL CHECK
══════════════════════════════════════════════════════════════════ */
function PayrollCheckModal({ emp, payslips, onClose }:{
  emp: Employee;
  ...
  const Row = ({ label, value, bold=false, red=false }:{ label:string; value:string; bold?:boolean; red?:boolean }) => (
    <div style={{ display:"flex", justifyContent:"space-between", fontSize:13.5, padding:"6px 0" }}>
      ...
      <span style={{ fontWeight:bold?700:600, color:red?"#dc2626":bold?"#1e1b4b":"#1e1b4b" }}>{value}</span>
    </div>
  );
        ...
        <div style={{ margin:"0 26px 20px", display:"flex", gap:8, overflowX:"auto", paddingBottom:4 }}>
          {payslips.map(p=>(
            <button key={p.id} onClick={()=>setSelected(p)}
              style={{ padding:"8px 14px", borderRadius:9, border:`1.5px solid ${selected.id===p.id?"#6366f1":"#e8e7f3"}`,
                background: selected.id===p.id?"#eef2ff":"#faf9ff",
                color: selected.id===p.id?"#6366f1":"#64748b",
                fontSize:13, fontWeight:600, cursor:"pointer", flexShrink:0, fontFamily:"inherit" }}>
              {p.month} {p.year}
            </button>
          ))}
        </div>
        ...
        <div style={{ padding:"0 26px 26px", display:"flex", flexDirection:"column", gap:16 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
            ...
            <span style={{ display:"inline-flex", alignItems:"center", gap:6, padding:"5px 12px", borderRadius:999, background: selected.paid?"#ecfdf5":"#fef2f2", color:selected.paid?"#059669":"#dc2626", fontSize:12.5, fontWeight:700 }}>
              {selected.paid ? "✅ Paid" : "⏳ Pending"} {selected.paid && `· ${selected.paidOn}`}
            </span>
          </div>
      ...
      <div style={{ ...MODAL_BOX, maxWidth:560, maxHeight:"92vh", overflowY:"auto" }}>
        ...
        <form onSubmit={handleSave} style={{ padding:"0 26px 26px" }}>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
            {([["Full Name","name","text"],["Employee ID","employeeId","text"],["Email","email","email"],["Phone","phone","text"],["Role","role","text"],["Hire Date","hireDate","text"]] as const).map(([label,key,type])=>(
              <div key={key}>
            ...
            <div>
              ...
              <input value={form.department.deptName} onChange={e=>set("department",e.target.value as Department)} style={inputSt}
                onFocus={e=>e.target.style.borderColor="#6366f1"} onBlur={e=>e.target.style.borderColor="#e8e7f3"} disabled/>
            ...
            <div>
              ...
              <input type="number" disabled value={employee.salary} onChange={e=>set("salary",Number(e.target.value))} style={inputSt}
                onFocus={e=>e.target.style.borderColor="#6366f1"} onBlur={e=>e.target.style.borderColor="#e8e7f3"} />
            </div>
            <div>
              <label style={labelSt}>Attendance Rate (%)</label>
              <input type="number" min={0} max={100} value={form.attendanceRate} onChange={e=>set("attendanceRate",Number(e.target.value))} style={inputSt}
                onFocus={e=>e.target.style.borderColor="#6366f1"} onBlur={e=>e.target.style.borderColor="#e8e7f3"} />
            </div>
          </div>
...
/* ── Selector Modal ─────────────────────────────────────────────── */
function SelectorModal({ employees, current, onSelect, onClose }:{
  employees:Employee[]; current:Employee|null; onSelect:(e:Employee)=>void; onClose:()=>void;
}) {
  const [q, setQ] = useState("");
  const filtered = useMemo(()=>employees.filter(e=>e.name.toLowerCase().includes(q.toLowerCase())||e.role.toLowerCase().includes(q.toLowerCase())),[employees,q]);
  return (
    <div onClick={e=>{ if(e.target===e.currentTarget) onClose(); }} style={OVERLAY}>
      <div style={{ ...MODAL_BOX, maxWidth:440, padding:"26px 24px" }}>
        <h2 style={{ margin:"0 0 14px", fontFamily:"'Outfit',sans-serif", fontWeight:700, fontSize:17, color:"#1e1b4b" }}>Select Employee</h2>
        <input value={q} onChange={e=>setQ(e.target.value)} placeholder="Search…" autoFocus
          style={{ ...inputSt, marginBottom:12 }} />
        <div style={{ maxHeight:320, overflowY:"auto", display:"flex", flexDirection:"column", gap:6 }}>
          {filtered.map(emp=>{
            const dc=DEPT_COLORS[emp.department]; const isCurrent=current?.id===emp.id;
            return (
              <button key={emp.id} onClick={()=>{ onSelect(emp); onClose(); }}
                style={{ display:"flex", alignItems:"center", gap:12, padding:"10px 12px", borderRadius:11, border:isCurrent?`1.5px solid ${dc.text}`:"1.5px solid #e8e7f3", background:isCurrent?dc.light:"#fff", cursor:"pointer", textAlign:"left", fontFamily:"inherit" }}>
                <div style={{ width:36, height:36, borderRadius:"50%", background:`linear-gradient(135deg,${dc.from},${dc.to})`, display:"flex", alignItems:"center", justifyContent:"center", color:"#fff", fontSize:12, fontWeight:700, flexShrink:0 }}>{getInitials(employee.name)}</div>
                <div style={{ minWidth:0 }}>
                  <p style={{ margin:0, fontSize:13.5, fontWeight:600, color:"#1e1b4b" }}>{emp.name}</p>
                  <p style={{ margin:0, fontSize:12, color:"#94a3b8" }}>{emp.role} · {emp.department}</p>
                </div>
        ...
        <button onClick={onClose} style={{ ...ghostBtn, width:"100%", marginTop:14 }}>Close</button>
      </div>
    </div>
  );
}
...
function PersonalTab({ emp }:{ emp:Employee }) {
  return (
    <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))", gap:14 }}>
      ...
      <InfoCard label="Email Address"  value={emp.email} />
      <InfoCard label="Phone Number"   value={emp.phone} />
      {/* <InfoCard label="Employee ID"    value={emp.employeeID} mono /> */}
    </div>
  );
}
  ...
  return (
    <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))", gap:14 }}>
      <InfoCard label="Department">
        <span style={{ display:"inline-flex", alignItems:"center", gap:7, fontSize:14, fontWeight:600, color:dc }}>
          <span style={{ width:9, height:9, borderRadius:"50%", background:dc, flexShrink:0 }} />{emp.department.deptName}
        </span>
      </InfoCard>
      <InfoCard label="Role"               value={emp.role} />
      <InfoCard label="Hire Date"          value={emp.hireDate} />
      {/* <InfoCard label="Employment Status"><StatusBadge status={emp.status} /></InfoCard> */}
    </div>
  );
}
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))", gap:14 }}>
        {[
          { label:"Attendance Rate", value:`${emp.attendanceRate}%`, bg:"#ecfdf5", border:"#bbf7d0", color:"#059669" },
          { label:"Days Present",    value:String(present),          bg:"#eff6ff", border:"#bfdbfe", color:"#2563eb" },
          { label:"Days Absent",     value:String(absent),           bg:"#fffbeb", border:"#fde68a", color:"#d97706" },
        ].map(s=>(
...
/* ── Leave History panel (inside Attendance tab area) ───────────── */
function LeaveHistoryPanel({ leaves }:{ leaves:LeaveRequest[] }) {
  if (!leaves.length) return null;

  return (
    <div style={{ marginTop:16, padding:"14px 16px", background:"#faf9ff", borderRadius:12, border:"1px solid #e8e7f3" }}>
      <p style={{ margin:"0 0 12px", fontSize:11, fontWeight:700, color:"#94a3b8", textTransform:"uppercase", letterSpacing:"0.06em" }}>Leave History</p>
      <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
        {leaves.map(l=>{
          const st = LEAVE_STATUS_STYLES[l.status];
          return (
            <div key={l.id} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:6 }}>
              <div>
                <span style={{ fontSize:13, fontWeight:600, color:"#1e1b4b" }}>{l.type}</span>
                <span style={{ fontSize:12, color:"#94a3b8", marginLeft:8 }}>{l.from} → {l.to} ({l.days}d)</span>
              </div>
              <span style={{ fontSize:11.5, fontWeight:600, padding:"2px 9px", borderRadius:999, background:st.bg, color:st.color }}>{l.status}</span>
            </div>
          );
        })}
      </div>
...
export default function EmployeeProfile() {
  ...
  const [current,   setCurrent]     = useState<Employee>(SEED[0]);
  ...
  const dc = DEPT_COLORS[current.department];

  const  userDataFetch=async(id)=>{ 
    const response = await fetch(`http://localhost:8080/employees/employee-by-id/${id}`,{method:"GET"})
    if(response.status==200){
      const userData=await response.json();
      setEmployee(userData);
      const imageSrc = `data:image/png;base64,${userData.photo}`;
      setImage(imageSrc);
    }
  }
  ...
  const handleLeaveSubmit = (r:LeaveRequest) => {
    setLeaves(prev=>[r,...prev]);
    setShowLeave(false);
    showToast(`Leave request submitted — ${r.days} day(s) of ${r.type} leave.`);
  };
  ...
  const tabContent: Record<Tab, React.ReactNode> = {
    ...
    attendance: (
      <>
        <AttendanceTab emp={employee} attendance={attendance} onCommit={()=>setShowAttend(true)} />
        <LeaveHistoryPanel leaves={leaves} />
      </>
    ),
  ...
  return (
    <div style={{ minHeight:"100vh", background:"linear-gradient(135deg,#f1f0fb 0%,#f5f3ff 60%,#fef9f3 100%)", fontFamily:"'DM Sans',sans-serif" }}>
        ...
        <div style={{ maxWidth:900, margin:"0 auto", display:"flex", alignItems:"center", justifyContent:"space-between", height:64, gap:12 }}>
          ...
          <div style={{ display:"flex", gap:8 }}>
            ...
            <button onClick={()=>navigate('/')}
              style={{ padding:"8px 16px", border:"none", borderRadius:10, color:"##6366f1", fontSize:13.5, fontWeight:600, cursor:"pointer", fontFamily:"inherit", boxShadow:"0 4px 12px rgba(99,102,241,0.3)" }}>
             Log Out
            </button>
          </div>
        </div>
        ...
        <div style={{ background:"#fff", borderRadius:18, border:"1px solid #e8e7f3", overflow:"hidden", marginBottom:20, boxShadow:"0 4px 20px rgba(99,102,241,0.07)" }}>
          ...
          <div style={{ padding:"0 24px 24px", marginTop:-44 }}>
            <div style={{ display:"flex", alignItems:"flex-end", gap:16, marginBottom:20, flexWrap:"wrap" }}>
              <div style={{ width:200, height:200, borderRadius:16, background:`linear-gradient(135deg,${dc.from},${dc.to})`, display:"flex", alignItems:"center", justifyContent:"center", color:"#fff", fontSize:26, fontWeight:800, border:"4px solid #fff", boxShadow:"0 8px 20px rgba(0,0,0,0.15)", flexShrink:0 }}>
                <img src={image} alt="Employee" style={{ width:"100%", height:"100%", objectFit:"cover", borderRadius:"10%" }} />
              </div>
              <div style={{ paddingBottom:4, flex:1, minWidth:0 }}>
                ...
                <div style={{ display:"flex", gap:10, flexWrap:"wrap", alignItems:"center" }}>
                  <span style={{ fontFamily:"monospace", fontSize:12, color:dc.text, background:dc.light, padding:"2px 8px", borderRadius:6, fontWeight:600 }}>{employee.employeeID}</span>
                  <StatusBadge status={employee.status} />
                </div>
              </div>
            </div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(120px,1fr))", gap:14, paddingTop:16, borderTop:"1px solid #e8e7f3", marginBottom:20 }}>
              {[
                ...
                { label:"Phone",      value:employee.phone || null },
            ...
            <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
              {[
                { label:"🌴 Request Leave",      color:"#059669", bg:"#ecfdf5", border:"#bbf7d0", onClick:()=>{ setActiveTab("attendance"); setShowLeave(true); } },
      ...
      {/* ── Modals ── */}
      {showSelector && <SelectorModal employees={employees} current={current} onSelect={e=>{ setCurrent(e); setLeaves(SEED_LEAVES); setAttendance(SEED_ATTENDANCE); }} onClose={()=>setShowSelector(false)} />}
      {showEdit     && <EditModal employee={employee} onClose={()=>setShowEdit(false)} onSave={handleSaveEmployee} />}
      {showLeave    && <RequestLeaveModal emp={employee} leaves={leaves} onClose={()=>setShowLeave(false)} onSubmit={handleLeaveSubmit} />}
